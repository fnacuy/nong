...
- 设置页改为「账号管理 / 默认方案 / 用户管理 / 激活卡密」四个标签页，「后台」入口移入页面底部。
- 统一设置页画布宽度，修复滚动条导致的页面偏移。
- 切换标签时自动调整滚动位置，避免标签被遮挡。
- 优化用户管理页随机输入按钮布局，避免长内容溢出。

### 提示样式优化

- 修复「自动捣乱」感叹号提示被下方卡片遮挡的问题。
- 感叹号底色改为红色警示色，提示文字位置微调，更易阅读。
