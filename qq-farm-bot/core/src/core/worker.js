...
// 初始化高级功能权限运行时标志（thread worker 从 workerData 注入；fork worker 走 FARM_PREMIUM_ACTIVE 环境变量回退）
if (parentPort && workerData && typeof workerData.premiumActive === 'boolean') {
    setPremiumActiveRuntime(workerData.premiumActive);
} else if (process.env.FARM_PREMIUM_ACTIVE === '1' || process.env.FARM_PREMIUM_ACTIVE === '0') {
    setPremiumActiveRuntime(process.env.FARM_PREMIUM_ACTIVE === '1');
}
...
async function runStarActivityAutoClaims() {
    ...
    if (isImmersiveFarmingActive()) return;
...
function runMysteryShopAutoBuy() {
    ...
    if (isImmersiveFarmingActive()) return Promise.resolve();
...
async function runFarmTick(autoConfig) {
    ...
    try {
        const immersive = isImmersiveFarmingActive();
        if (autoConfig.farm && !immersive) await checkFarm();
        if (autoConfig.task && !immersive) await checkAndClaimTasks();
        if (autoConfig.email && !immersive) await checkAndClaimEmails();
        if (autoConfig.fertilizer_gift && !immersive) await openFertilizerGiftPacksSilently();
    } catch { } finally {
...
async function runHelpTick(autoConfig) {
    ...
    try {
        ...
        if (autoConfig.friend_golden_bug && !immersive) await runGoldenBugPlacement();
    } catch (err) {
        if (!isTransientNetworkError(err)) {
...
onMasterMessage(async (msg) => {
    try {
        ...
        } else if (msg.type === 'premium_status') {
            setPremiumActiveRuntime(msg.active === true);
        }
    } catch (err) {
...
async function handleApiCall(msg) {
    ...
    try {
        switch (method) {
            ...
            case 'getDogGifts': {
                const { getDogGifts } = require('../services/activity');
                result = await getDogGifts();
                break;
            }
            case 'getDogInfo': {
                const { getDogInfo } = require('../services/dog');
                result = await getDogInfo();
                break;
            }
            case 'setDogActive': {
                const { setDogActive } = require('../services/dog');
                result = await setDogActive(args[0], args[1]);
                break;
            }
            case 'getPetStatus': {
                const { getDogStatus } = require('../services/pet');
                result = await getDogStatus(args[0] || 0);
                break;
            }
            case 'getPetList': {
                const { getPetList } = require('../services/pet');
                result = await getPetList();
                break;
            }
            case 'activateDog': {
                const { activateDog } = require('../services/pet');
                result = await activateDog(args[0]);
                break;
            }
            case 'deployDog': {
                const { deployDog } = require('../services/pet');
                result = await deployDog(args[0]);
                break;
            }
            case 'withdrawDog': {
                const { withdrawDog } = require('../services/pet');
                result = await withdrawDog();
                break;
            }
            case 'getDogFoodList': {
                const { getDogFoodList } = require('../services/pet');
                result = await getDogFoodList();
                break;
            }
            case 'feedDog': {
                const { feedDog } = require('../services/pet');
                result = await feedDog(args[0], args[1]);
                break;
            }
            case 'getGuardLogs': {
                const { getGuardLogs } = require('../services/pet');
                result = await getGuardLogs(args[0], args[1]);
                break;
            }
            case 'getGuardReward': {
                const { getGuardReward } = require('../services/pet');
                result = await getGuardReward();
                break;
            }
            case 'claimGuardReward': {
                const { claimGuardReward } = require('../services/pet');
                result = await claimGuardReward();
                break;
            }
            case 'claimDogGifts': {
                const { claimDogGifts } = require('../services/activity');
            ...
            case 'rawRpc': {
                ...
                const body = args[2] || Buffer.from([]);
                try {
                    const { body: replyBody } = await sendMsgAsync(service, methodName, body, 10000);
                    result = { ok: true, len: replyBody ? replyBody.length : 0, hex: replyBody ? replyBody.toString('hex').slice(0, 300) : '' };
                } catch (e) {
