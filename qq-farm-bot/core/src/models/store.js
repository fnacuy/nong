...
/** 资本模式可选上阵的狗狗 ID（田园犬 / 牧羊犬 / 斑点狗 / 柯基 / 护主犬） */
const CAPITAL_MODE_DOG_IDS = new Set([90001, 90002, 90003, 90011, 90021]);
...
/** 默认沉浸式务农时段（时段内仅保留好友帮助，暂停其余自动化） */
const DEFAULT_IMMERSIVE_FARMING = {
    enabled: false,
    start: '00:00',
    end: '06:00'
};
...
/** 高级静谧模式允许的模式枚举 */
const PREMIUM_QUIET_MODE_SET = new Set(['no_steal_no_help', 'no_steal', 'no_help']);

/** 默认高级静谧模式（多时段，仅调整偷菜/帮忙，不影响捣乱） */
const DEFAULT_PREMIUM_QUIET_MODE = {
    enabled: false,
    slots: []
};
...
const DEFAULT_ACCOUNT_CONFIG = {
    ...
    immersiveFarming: DEFAULT_IMMERSIVE_FARMING,
    premiumQuietMode: DEFAULT_PREMIUM_QUIET_MODE,
    knownFriendGids: [],
    friendBlacklist: [],
...
function normalizePremiumQuietMode(raw) {
    const input = raw && typeof raw === 'object' ? raw : {};
    const slots = [];
    ...
    for (let i = 0; i < rawSlots.length; i++) {
        const item = rawSlots[i] && typeof rawSlots[i] === 'object' ? rawSlots[i] : {};
        const mode = String(item.mode || '').trim();
        if (!PREMIUM_QUIET_MODE_SET.has(mode)) continue;

        const start = normalizeTimeString(item.start, '22:00');
        const end = normalizeTimeString(item.end, '06:00');

        let id = String(item.id || '').trim();
        if (!id) id = `s${  i + 1}`;
        if (seenIds.has(id)) id = `s${  i + 1  }_${  slots.length}`;
        seenIds.add(id);

        slots.push({ id, mode, start, end });
    }
...
function cloneAccountConfig(config = DEFAULT_ACCOUNT_CONFIG) {
    ...
    return {
        ...
        immersiveFarming: { ...config.immersiveFarming || DEFAULT_ACCOUNT_CONFIG.immersiveFarming },
        premiumQuietMode: normalizePremiumQuietMode(config.premiumQuietMode),
        knownFriendGids,
...
let accountFallbackConfig = (() => {
    ...
    cfg.immersiveFarming = { ...DEFAULT_ACCOUNT_CONFIG.immersiveFarming };
    cfg.premiumQuietMode = normalizePremiumQuietMode(DEFAULT_ACCOUNT_CONFIG.premiumQuietMode);
    cfg.knownFriendGids = [];
...
function normalizeAccountConfig(raw, fallbackConfig = accountFallbackConfig) {
    ...
    if (input.automation && typeof input.automation === 'object') {
        for (const [key, value] of Object.entries(input.automation)) {
            ...
            } else if (key === 'capital_mode_guard_seconds') {
                cfg.automation[key] = Math.max(5, Math.min(300, Number(value) || 10));
            } else if (key === 'capital_mode_dog_id') {
                const dogId = Number.parseInt(value, 10);
                cfg.automation[key] = CAPITAL_MODE_DOG_IDS.has(dogId) ? dogId : 0;
            } else {
    ...
    cfg.qingmeiClaimDate = /^\d{4}-\d{2}-\d{2}$/.test(String(input.qingmeiClaimDate || ''))
        ? String(input.qingmeiClaimDate) : '';
    ...
    // 沉浸式务农时段
    if (input.immersiveFarming && typeof input.immersiveFarming === 'object') {
        const prev = cfg.immersiveFarming || {};
        cfg.immersiveFarming = {
            enabled: input.immersiveFarming.enabled !== undefined
                ? !!input.immersiveFarming.enabled : !!prev.enabled,
            start: normalizeTimeString(input.immersiveFarming.start, prev.start || '00:00'),
            end: normalizeTimeString(input.immersiveFarming.end, prev.end || '06:00')
        };
    }

    // 高级静谧模式（多时段）
    if (input.premiumQuietMode && typeof input.premiumQuietMode === 'object') {
        cfg.premiumQuietMode = normalizePremiumQuietMode(input.premiumQuietMode);
    }
...
function pickDefaultPlanConfig(raw) {
    ...
    return {
        ...
        immersiveFarming: { ...cfg.immersiveFarming },
        premiumQuietMode: normalizePremiumQuietMode(cfg.premiumQuietMode),
...
function loadGlobalConfig() {
    ...
    try {
        ...
        // 失效 Code 定期清理配置
        if (data.codeCleanup && typeof data.codeCleanup === 'object') {
            globalConfig.codeCleanup = {
                enabled: data.codeCleanup.enabled === true,
                retainDays: Math.max(1, Number.parseInt(String(data.codeCleanup.retainDays), 10) || DEFAULT_CODE_CLEANUP_CONFIG.retainDays),
                failThreshold: Math.max(1, Number.parseInt(String(data.codeCleanup.failThreshold), 10) || DEFAULT_CODE_CLEANUP_CONFIG.failThreshold),
                intervalHours: Math.max(1, Number.parseInt(String(data.codeCleanup.intervalHours), 10) || DEFAULT_CODE_CLEANUP_CONFIG.intervalHours)
            };
        }
    } catch (err) {
...
function getConfigSnapshot(accountId) {
    ...
    return {
        ...
        qingmeiClaimDate: String(cfg.qingmeiClaimDate || ''),
        intervals: { ...cfg.intervals },
        friendQuietHours: { ...cfg.friendQuietHours },
        immersiveFarming: { ...cfg.immersiveFarming },
        premiumQuietMode: normalizePremiumQuietMode(cfg.premiumQuietMode),
...
function applyConfigSnapshot(patch = {}, opts = {}) {
    ...
    if (patch.qingmeiClaimDate !== undefined && patch.qingmeiClaimDate !== null) {
        const claimDate = String(patch.qingmeiClaimDate || '');
        cfg.qingmeiClaimDate = /^\d{4}-\d{2}-\d{2}$/.test(claimDate) ? claimDate : '';
    }
    ...
    if (patch.immersiveFarming && typeof patch.immersiveFarming === 'object') {
        const prev = cfg.immersiveFarming || {};
        cfg.immersiveFarming = {
            enabled: patch.immersiveFarming.enabled !== undefined
                ? !!patch.immersiveFarming.enabled : !!prev.enabled,
            start: normalizeTimeString(patch.immersiveFarming.start, prev.start || '00:00'),
            end: normalizeTimeString(patch.immersiveFarming.end, prev.end || '06:00')
        };
    }
    if (patch.premiumQuietMode && typeof patch.premiumQuietMode === 'object') {
        cfg.premiumQuietMode = normalizePremiumQuietMode(patch.premiumQuietMode);
    }
...
function getQingmeiClaimDate(accountId) {
    return String(getAccountConfigSnapshot(accountId).qingmeiClaimDate || '');
}

function setQingmeiClaimDate(accountId, date) {
    const claimDate = /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? String(date) : '';
    const result = applyConfigSnapshot({ qingmeiClaimDate: claimDate }, { accountId });
    return String(result.qingmeiClaimDate || '');
}
...
function getImmersiveFarming(accountId) {
    return { ...getAccountConfigSnapshot(accountId).immersiveFarming };
}

function getPremiumQuietMode(accountId) {
    return normalizePremiumQuietMode(getAccountConfigSnapshot(accountId).premiumQuietMode);
}

// ==================== 高级功能权限（运行时注入） ====================

let premiumActiveRuntime = null;

function setPremiumActiveRuntime(active) {
    premiumActiveRuntime = !!active;
}

function getPremiumActive() {
    if (typeof premiumActiveRuntime === 'boolean') return premiumActiveRuntime;
    if (process.env.FARM_PREMIUM_ACTIVE === '1') return true;
    if (process.env.FARM_PREMIUM_ACTIVE === '0') return false;
    return false;
}

function parseTimeToMinutesInternal(timeStr) {
...
/** 沉浸式务农时段内：仅保留好友帮助，暂停其余自动化。无高级功能权限时不生效。 */
function isImmersiveFarmingActive(now = new Date()) {
    if (!getPremiumActive()) return false;
    const cfg = getImmersiveFarming();
    if (!cfg || !cfg.enabled) return false;
    const startMin = parseTimeToMinutesInternal(cfg.start);
    const endMin = parseTimeToMinutesInternal(cfg.end);
...
/**
 * 高级静谧模式：返回当前命中的时段模式（无权限/未启用/未命中返回 null）。
 * 多时段按配置顺序，首个命中优先；不影响捣乱（doBad）。
 * @returns {string|null} 'no_steal_no_help' | 'no_steal' | 'no_help' | null
 */
function getActivePremiumQuietMode(now = new Date()) {
    if (!getPremiumActive()) return null;
    const cfg = getPremiumQuietMode();
    if (!cfg || !cfg.enabled || !Array.isArray(cfg.slots) || cfg.slots.length === 0) return null;

    const currentMin = now.getHours() * 60 + now.getMinutes();
    for (const slot of cfg.slots) {
        if (!slot || !PREMIUM_QUIET_MODE_SET.has(slot.mode)) continue;
        const startMin = parseTimeToMinutesInternal(slot.start);
        const endMin = parseTimeToMinutesInternal(slot.end);
        ...
        if (startMin === endMin) {
            ...
            hit = currentMin >= startMin && currentMin < endMin;
        } else {
            hit = currentMin >= startMin || currentMin < endMin;
        }
        if (hit) return slot.mode;
    }
    return null;
}
...
function addOrUpdateAccount(account) {
    ...
    const nextOpenId = account.openId || account.open_id || account.yybOpenid || account.yybOpenId;
    ...
    if (account.id) {
        ...
        data.accounts.push({
            ...
            yybOpenid: account.yybOpenid || account.yybOpenId
                ? String(account.yybOpenid || account.yybOpenId)
                : (nextOpenId ? String(nextOpenId) : ''),
            avatar: nextAvatar || '',
...
const DEFAULT_CODE_CLEANUP_CONFIG = {
    enabled: false,
    retainDays: 7,
    failThreshold: 3,
    intervalHours: 24
...
function getCodeCleanupConfig() {
    return globalConfig.codeCleanup
        ? { ...DEFAULT_CODE_CLEANUP_CONFIG, ...globalConfig.codeCleanup }
        : { ...DEFAULT_CODE_CLEANUP_CONFIG };
}

function setCodeCleanupConfig(config) {
    if (!config || typeof config !== 'object') return null;
    const current = getCodeCleanupConfig();
    globalConfig.codeCleanup = {
        enabled: config.enabled === true,
        retainDays: Math.max(1, Number.parseInt(String(config.retainDays !== undefined ? config.retainDays : current.retainDays), 10) || DEFAULT_CODE_CLEANUP_CONFIG.retainDays),
        failThreshold: Math.max(1, Number.parseInt(String(config.failThreshold !== undefined ? config.failThreshold : current.failThreshold), 10) || DEFAULT_CODE_CLEANUP_CONFIG.failThreshold),
        intervalHours: Math.max(1, Number.parseInt(String(config.intervalHours !== undefined ? config.intervalHours : current.intervalHours), 10) || DEFAULT_CODE_CLEANUP_CONFIG.intervalHours)
    };
    saveGlobalConfig();
    return { ...globalConfig.codeCleanup };
}
...
module.exports = {
    ...
    getFriendQuietHours,
    getImmersiveFarming,
    isImmersiveFarmingActive,
    getPremiumQuietMode,
    getActivePremiumQuietMode,
    getPremiumActive,
    setPremiumActiveRuntime,
    getKnownFriendGids,
    setKnownFriendGids,
    getFriendBlacklist,
    setFriendBlacklist,
    ...
    getCodeCleanupConfig,
    setCodeCleanupConfig,
    DEFAULT_CODE_CLEANUP_CONFIG
};
