...
function registerUser(username, password, cardCode) {
    ...
    if (cardType === 'premium') {
        return { ok: false, error: '高级功能卡请在登录后于续费/激活中使用' };
    }
...
/**
 * 激活/叠加一种"按时长授权"的权限字段（极速务农 turbo / 高级功能 premium）。
 * 复用相同的叠加规则：永久卡、已永久跳过、到期时间续叠。
 * @param {object} user - 用户记录
 * @param {object} card - 卡密记录
 * @param {string} field - 用户对象上的字段名（'turbo' 或 'premium'）
 * @param {number} now - 当前时间戳
 */
function applyTimedEntitlement(user, card, field, now) {
    const duration = normalizeTimeDuration(card);
    if (!user[field]) {
        user[field] = {
            code: card.code,
    ...
    const ent = user[field];
    const prevExpiresAt = ent.expiresAt || 0;
    const prevDuration = normalizeTimeDuration({ ...ent, allowZero: true });

    if (duration.isPermanent) {
        ent.expiresAt = null;
        assignDurationFields(ent, duration);
    } else if (prevDuration.isPermanent) {
        ent.expiresAt = null;
    } else {
        const totalMs = (prevDuration.durationMs || 0) + duration.durationMs;
        const totalDuration = normalizeTimeDuration({
            durationMs: totalMs,
            durationUnit: totalMs % DAY_MS === 0 ? 'day' : 'hour'
        });
        assignDurationFields(ent, totalDuration);
        if (prevExpiresAt && prevExpiresAt > now) {
            ent.expiresAt = prevExpiresAt + duration.durationMs;
        } else {
            ent.expiresAt = now + duration.durationMs;
        }
    }
}
...
function renewUser(username, cardCode) {
    ...
    if (cardType === 'quota') {
        ...
        // 极速务农卡：激活/叠加极速务农权限时长
        applyTimedEntitlement(user, card, 'turbo', now);
    } else if (cardType === 'premium') {
        // 高级功能卡：永久卡可直接激活；天数/限时卡每人仅限激活一次
        const premiumDuration = normalizeTimeDuration(card);
        if (!premiumDuration.isPermanent && user.premium) {
            return { ok: false, error: '高级功能天数卡每人仅限激活一次，永久卡不受此限制' };
        }
        applyTimedEntitlement(user, card, 'premium', now);
    } else {
        // 时间卡密
        const duration = normalizeTimeDuration(card);
        if (!user.card) {
...
function getUserPremiumStatus(username) {
    ...
    const premium = user.premium || null;
    if (!premium) return { active: false, expiresAt: null, isPermanent: false };

    if (premium.isPermanent === true || premium.days === -1 || premium.durationValue === -1) {
        return { active: true, expiresAt: null, isPermanent: true };
    }

    const expiresAt = Number(premium.expiresAt) || 0;
    const active = expiresAt > Date.now();
    return {
        active,
        expiresAt: active ? expiresAt : null,
...
function createCard(description, days, type = 'time', options = {}) {
    loadCards();
    const cardType = type === 'quota' ? 'quota' : type === 'turbo' ? 'turbo' : type === 'premium' ? 'premium' : 'time';
    const card = {
