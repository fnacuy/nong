...
function registerAdminFarmResourceRoutes({
  ...
  app.get("/api/dog/info", async (req, res) => {
    ...
    try {
      const result = await provider.getDogInfo(accountId);
      res.json({ ok: true, data: result });
    }
    catch (error) {
      sendProviderError(res, error);
  ...
  app.post("/api/dog/active", async (req, res) => {
    const accountId = requireAccessibleAccount(req, res, getAccountIdFromRequest, canAccessAccount);
    if (!accountId)
      return;

    try {
      const { dogId, active } = req.body || {};
      const result = await provider.setDogActive(accountId, Number(dogId) || 0, active ? 1 : 0);
      res.json({ ok: true, data: result });
    }
    catch (error) {
      sendProviderError(res, error);
  ...
  app.get("/api/dog/gifts", async (req, res) => {
    ...
    try {
      const result = await provider.getDogGifts(accountId);
      res.json({ ok: true, data: result });
    }
    catch (error) {
      sendProviderError(res, error);
  ...
  app.post("/api/dog/gifts/claim", async (req, res) => {
    ...
    try {
      const result = await provider.claimDogGifts(accountId);
      res.json(result);
    }
    catch (error) {
      sendProviderError(res, error);
