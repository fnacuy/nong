...
function addCapturedValues(flow, snapshot) {
  ...
  flow.rpcCalls = Array.isArray(data.rpcCalls) ? data.rpcCalls : (flow.rpcCalls || []);
...
function serializeFlow(flow) {
  ...
  return {
    ...
    rpcCalls: Array.isArray(flow.rpcCalls) ? flow.rpcCalls.slice(-200) : [],
  };
}
      ...
      try {
        const started = await captureRequest(config, "/api/capture/start", {
          ...
          body: {
            ...
            advertiseHost: isEmbeddedMode() ? "8.149.133.128" : "",
          },
          timeout: 30_000,
        });
