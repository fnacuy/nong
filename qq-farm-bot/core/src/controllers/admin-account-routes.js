...
function registerAdminAccountRoutes({
  ...
  app.get("/api/admin/accounts/code-cleanup-config", requireSuperAdminRole, (req, res) => {
    try {
      if (!expiredCodeCleaner) {
        return res.status(500).json({ ok: false, error: "清理服务不可用" });
      }
      res.json({ ok: true, data: expiredCodeCleaner.getConfig() });
    } catch (error) {
      res.status(500).json({ ok: false, error: error.message });
    }
  });

  app.post("/api/admin/accounts/code-cleanup-config", requireSuperAdminRole, (req, res) => {
    try {
      if (!expiredCodeCleaner) {
        return res.status(500).json({ ok: false, error: "清理服务不可用" });
      }
      if (!requireDangerConfirmation(req, res, "UPDATE_CODE_CLEANUP_CONFIG")) {
        return;
      }
      const body = req.body && typeof req.body === "object" ? req.body : {};
      const config = expiredCodeCleaner.getConfig();
      const nextConfig = {
        enabled: body.enabled !== undefined ? body.enabled === true : config.enabled,
        retainDays: body.retainDays !== undefined ? Number(body.retainDays) : config.retainDays,
        failThreshold: body.failThreshold !== undefined ? Number(body.failThreshold) : config.failThreshold,
        intervalHours: body.intervalHours !== undefined ? Number(body.intervalHours) : config.intervalHours,
      };
      if (
        !Number.isInteger(nextConfig.retainDays)
        || nextConfig.retainDays < 1
        || !Number.isInteger(nextConfig.failThreshold)
        || nextConfig.failThreshold < 1
        || !Number.isInteger(nextConfig.intervalHours)
        || nextConfig.intervalHours < 0
      ) {
        return res.status(400).json({ ok: false, error: "配置参数非法：保留时限与失败阈值须为正整数，清理间隔须为非负整数" });
      }
      const saved = store.setCodeCleanupConfig({
        enabled: nextConfig.enabled,
        retainDays: nextConfig.retainDays,
        failThreshold: nextConfig.failThreshold,
        intervalHours: nextConfig.intervalHours,
      });
      expiredCodeCleaner.reschedule();
      res.json({ ok: true, data: { ...saved, lastRunAt: config.lastRunAt, lastCleanupCount: config.lastCleanupCount } });
    } catch (error) {
      res.status(500).json({ ok: false, error: error.message });
    }
  });

  app.post("/api/admin/accounts/cleanup-expired-codes", requireSuperAdminRole, async (req, res) => {
    try {
      if (!expiredCodeCleaner) {
        return res.status(500).json({ ok: false, error: "清理服务不可用" });
      }
      if (!requireDangerConfirmation(req, res, "CLEAN_EXPIRED_CODES")) {
        return;
      }
      const result = await expiredCodeCleaner.runCleanup();
