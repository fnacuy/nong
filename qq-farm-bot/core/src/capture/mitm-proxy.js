 ...
 * @param {object} deps.rpcExtractor - createRpcExtractor() 的结果（记录 RPC 方法名）
 * @param {object} deps.sessionStore - 会话存储
 * @param {Function} deps.log - (level, message, extra)
 */
function createMitmProxyManager(deps = {}) {
  const { config, ca, friendExtractor, rpcExtractor, sessionStore, log = () => {} } = deps;
  ...
  function startMitm(rawSocket, { host, port }, session) {
    ...
    clientTls.on('data', (chunk) => {
      ...
      if (headParsed) {
        if (wsClientParser) wsClientParser.push(chunk);
      ...
      // 升级为 WebSocket 后解析好友 GID + 记录 RPC 方法名
      if (parsed && parsed.isUpgrade && friendExtractor) {
        wsParser = new WsFrameParser({
          onMessage: (message) => {
            try {
              if (rpcExtractor) {
                const rpc = rpcExtractor.handleMessage(message);
                if (rpc) sessionStore.addRpcCall(session, rpc);
              }
      ...
      // 客户端→服务器方向：解析掩码帧，记录请求方法名
      if (parsed && parsed.isUpgrade && rpcExtractor) {
        wsClientParser = new WsFrameParser({
          onMessage: (message) => {
            try {
              const rpc = rpcExtractor.handleMessage(message);
              if (rpc) sessionStore.addRpcCall(session, rpc);
        ...
        if (rest.length > 0) wsClientParser.push(rest);
      }
  ...
  function handlePlainHttp(rawSocket, { host, port, path }, head, rest, session, bypassHosts) {
    ...
    upstream.once('connect', () => {
      ...
      const rewritten = [[parts[0], path || '/', parts[2] || 'HTTP/1.1'].join(' '), ...lines.slice(1)].join('\r\n');
