...
function createSessionStore({ config } = {}) {
  ...
  /** 记录抓到的 RPC 调用（service + method + 方向），用于协议分析 */
  function addRpcCall(session, { service = '', method = '', messageType = 0 } = {}) {
    const cleanService = String(service || '').trim();
    const cleanMethod = String(method || '').trim();
    if (!cleanService || !cleanMethod) return false;
    const entry = {
    ...
    const last = session.rpcCalls[session.rpcCalls.length - 1];
    if (last && last.service === entry.service && last.method === entry.method && last.messageType === entry.messageType) {
      return false;
    }
    session.rpcCalls.push(entry);
    if (session.rpcCalls.length > 2000) session.rpcCalls.shift();
    markUpdated(session);
    return true;
  }
  ...
  function buildSnapshot(session) {
    return {
      ...
      rpcCalls: session.rpcCalls.map(c => ({ service: c.service, method: c.method, messageType: c.messageType, ts: c.ts })),
    };
  }
