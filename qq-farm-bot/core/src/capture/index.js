...
const { createRpcExtractor } = require('./rpc-extractor');
...
function createCaptureCore(options = {}) {
  ...
  const ready = (async () => {
    ...
    const rpcExtractor = await createRpcExtractor();
    proxyManager = createMitmProxyManager({ config, ca, friendExtractor, rpcExtractor, sessionStore, log });
