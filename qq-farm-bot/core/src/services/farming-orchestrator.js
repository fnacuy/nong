...
const { isAutomationOn, getAutomation, getPrioritize2x2Crops, isImmersiveFarmingActive } = require('../models/store');
...
const { checkCapitalModeOnFarm, scheduleCapitalDogRecall, resetCapitalModeState } = require('./dog');
...
async function checkFarm() {
  ...
  if (isCheckingFarm || !userState.gid || !isAutomationOn('farm') || !isConnected() || isImmersiveFarmingActive()) {
    return false;
  }
...
async function runFarmOperation(opType) {
  ...
  if (opType === 'all') {
    try {
      await checkCapitalModeOnFarm(lands);
    } catch (err) {
      logWarn('资本模式', `成熟前放狗检查失败: ${err.message}`, {
        module: 'farm', event: '资本模式放狗', result: 'error'
      });
    }
  }
