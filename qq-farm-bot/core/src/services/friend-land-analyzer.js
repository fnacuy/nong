  ...
  try {
    ...
    const friends = rawFriends
      .filter(friend => {
        ...
        // Exclude entries without a readable name (removed/invalid friends)
        if (!sanitizeFriendName(friend.name) && !sanitizeFriendName(friend.remark)) {
          return false;
        }
        return true;
      })
      .map(friend => {
        ...
        return {
          gid,
          name: sanitizeFriendName(friend.remark) || sanitizeFriendName(friend.name) || `GID:${gid}`,
