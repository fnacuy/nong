...
const { getQingmeiClaimDate, setQingmeiClaimDate } = require('../models/store');
...
function getQingmeiAccountId() {
  return String(process.env.FARM_ACCOUNT_ID || '');
}

function markQingmeiClaimedToday() {
  const dateKey = getLocalDateKey();
  qingmeiClaimedDateByAccount.set(getQingmeiClaimStateKey(), dateKey);
  try {
    setQingmeiClaimDate(getQingmeiAccountId(), dateKey);
  } catch (_) {
    // 持久化失败不影响内存态
  }
}
...
function isQingmeiClaimedToday() {
  if (qingmeiClaimedDateByAccount.get(getQingmeiClaimStateKey()) === getLocalDateKey()) {
    return true;
  }
  try {
    const persisted = getQingmeiClaimDate(getQingmeiAccountId());
    return persisted === getLocalDateKey();
  } catch (_) {
    return false;
  }
}
...
function buildGuanxingView(starRecord) {
  const records = starRecord?.records || [];
  const nodes = records.map(record => ({
    ...
    statusLabel: record.claimed ? '已点亮' : record.claimable ? '可点亮' : record.unlocked ? '已解锁' : '未解锁',
    rewards: record.rewards || [],
  }));
  const currentDay = toNum(starRecord?.status) || toNum(starRecord?.openedDays) || toNum(starRecord?.unlockedCount) || 0;
  return {
    nodes,
    summary: {
      totalDays: toNum(starRecord?.totalCount) || nodes.length,
      currentDay,
      unlockedCount: toNum(starRecord?.unlockedCount) || 0,
      claimedCount: toNum(starRecord?.claimedCount) || 0,
      claimableCount: toNum(starRecord?.claimableCount) || 0,
      pendingRewards: records.filter(record => record.claimable).flatMap(record => record.rewards || []),
    },
  };
}

async function getStarActivity() {
  const offlineStarRecord = normalizeStarRecord(null);
  const offlineGuanxingView = buildGuanxingView(offlineStarRecord);
  if (!getUserState()) {
    return {
      ...
      ...offlineGuanxingView,
      currentDay: offlineGuanxingView.summary.currentDay,
      totalDays: offlineGuanxingView.summary.totalDays,
      seasonTitle: '心许千灯星垂野',
  ...
  const guanxingView = buildGuanxingView(starRecord);
  ...
  return {
    ...
    solarTerms,
    qingmei,
    nodes: guanxingView.nodes,
    summary: {
      ...guanxingView.summary,
      starCount: starRecord.totalCount,
      exchangeShopCount: shopItems.length,
    },
    currentDay: guanxingView.summary.currentDay,
    totalDays: starRecord.totalCount,
    seasonTitle: '心许千灯星垂野',
    nowTime: Math.floor(Date.now() / 1000),
    heluBalance: starSandBalance,
  };
}

/**
 * 获取护主犬信息（同气礼包数量依赖 GetDogInfo）。
 * 后端游戏协议中“可领取同气礼包”字段尚不明确，暂返回 0，保留完整狗信息供展示。
 */
async function getDogGifts() {
  const state = getUserState();
  const connected = !!(state && isConnected());
  const base = { ok: true, claimable: 0, connected, dogs: [], coin: 0, protectTime: 0, foods: [] };
  if (!connected) {
  ...
  try {
    ...
    return { ...base, ok: false, error: err?.message || '获取护主犬信息失败' };
  }
}

/**
 * 领取同气礼包。ClaimSkillGifts 协议尚未完整定义，返回未接入提示。
 */
async function claimDogGifts() {
  ...
  return { ok: false, error: '同气礼包领取功能尚未接入，请等待后续版本' };
}
