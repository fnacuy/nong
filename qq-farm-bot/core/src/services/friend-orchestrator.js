...
async function checkFriends(options = {}) {
  ...
  const immersiveActive = isImmersiveFarmingActive();
  const premiumActive = getPremiumActive();
  const quietMode = premiumActive ? getActivePremiumQuietMode() : null;

  let doHelp = onlyHelp ? true : (onlySteal || onlyBad ? false : helpEnabled);
  let doSteal = immersiveActive ? false : (onlySteal ? true : (onlyHelp || onlyBad ? false : stealEnabled));
  const doBad = immersiveActive ? false : (onlyBad ? true : (onlyHelp || onlySteal ? false : badEnabled));

  // 高级静谧模式：仅调整偷菜/帮忙，捣乱（doBad）不受影响
  if (quietMode === 'no_steal_no_help') {
    doSteal = false;
    doHelp = false;
  } else if (quietMode === 'no_steal') {
    doSteal = false;
  } else if (quietMode === 'no_help') {
    doHelp = false;
  }
  ...
  try {
    ...
    const turboModeEnabled = !!isAutomationOn('friend_turbo_mode');
    const turboActive = turboModeEnabled && premiumActive;
    ...
    // Help targets
    if (doHelp) {
      if ((helpExpReached && guardDogGidSet.size > 0) || turboActive) {
        // Experience limit reached (or turbo mode enabled) — only help guard dog friends
        for (const friend of rawFriends) {
          const gid = toNum(friend.gid);
          if (gid === userState.gid) continue;
      ...
      for (const target of helpTargets) {
        try {
          await visitFriendForHelp(
            ...
            ignoreExpLimit, helpExpReached || turboActive
          );
        } catch {
          // Skip individual failures
        }
        await randomDelay(
          (helpExpReached || turboActive) ? 300 : 500,
          (helpExpReached || turboActive) ? 700 : 1000
        );
      }
    }
