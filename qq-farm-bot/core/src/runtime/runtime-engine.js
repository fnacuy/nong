...
const { createExpiredCodeCleaner } = require('./expired-code-cleaner');
...
function createRuntimeEngine(options = {}) {
    ...
    const {
        ...
        onCodeExpired: (accountId, reason) => autoCodeRefresh.refreshAccountCode(accountId, reason),
    ...
    // 创建失效 Code 清理服务
    const expiredCodeCleaner = createExpiredCodeCleaner({
        store,
        provider: {
            getAccounts: store.getAccounts,
            stopWorker,
            addAccountLog,
            isAccountRunning: (id) => !!(id && workers[id])
        },
        logger: {
            info: (msg, meta) => log('系统', msg, meta),
            warn: (msg, meta) => log('系统', msg, meta)
        }
    });
    ...
    dataProvider.expiredCodeCleaner = expiredCodeCleaner;
