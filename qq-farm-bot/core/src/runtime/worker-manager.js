...
function createWorkerManager(deps) {
    ...
    // code 过期自动刷新去重：记录每个账号最近一次触发时间，避免重复刷新
    const lastCodeExpiredTriggerAt = new Map();
    const CODE_EXPIRED_DEDUP_MS = 5 * 60 * 1000;

    /**
     * 触发 code 过期自动刷新
     * 刷新成功会由 refreshAccountCode 内部重启 Worker；失败则停止账号。
     */
    function triggerCodeExpired(accountId, reason) {
        const now = Date.now();
        const last = lastCodeExpiredTriggerAt.get(accountId) || 0;
        if (now - last < CODE_EXPIRED_DEDUP_MS) return;
        lastCodeExpiredTriggerAt.set(accountId, now);

        const wrk = workers[accountId];
        const accountName = wrk ? wrk.name : '';

        log('系统', `账号 ${  accountName || accountId  } Code 已失效，尝试自动刷新`, {
            accountId: String(accountId),
            accountName,
            reason,
        });

        if (typeof onCodeExpired !== 'function') {
            stopWorker(accountId);
            return;
        }

        Promise.resolve()
            .then(() => onCodeExpired(accountId, reason))
            .then((ok) => {
                if (ok === true) return;
                log('系统', `账号 ${  accountName || accountId  } Code 自动刷新失败，停止账号`, {
                    accountId: String(accountId),
                    accountName,
                    reason,
                });
    ...
    /** 查询账号所属用户的高级功能权限是否激活（管理员/超管恒为 true） */
    function resolvePremiumActive(account) {
        try {
            const username = String(account && account.username || '').trim();
            if (!username) return false;
            const { getUserPremiumStatus } = require('../models/user-store');
            return getUserPremiumStatus(username).active === true;
        } catch {
            return false;
        }
    }

    /** 用户激活/续费高级功能卡密后，向该用户名下所有运行中的 worker 推送最新权限标志 */
    function broadcastPremiumStatus(username) {
    ...
    function createThreadWorker(account) {
        const worker = new WorkerThread(workerScriptPath, {
            workerData: {
                ...
                channel: 'thread',
                premiumActive: resolvePremiumActive(account)
            }
        });
        // 统一 send/kill 接口
        ...
        if (processRef.pkg) {
            ...
            return fork(mainEntryPath, [], {
                ...
                env: {
                    ...
                    FARM_PREMIUM_ACTIVE: resolvePremiumActive(account) ? '1' : '0'
                }
            });
        }
    ...
    function handleWorkerMessage(accountId, msg) {
        ...
        if (msg.type === 'status_sync') {
            ...
            // Code 400 = 登录失效
            if (code === 400) {
                addAccountLog('ws_400', `账号 ${  wrk.name  } 登录失效，尝试自动刷新 Code`,
                    accountId, wrk.name);
                triggerCodeExpired(accountId, 'ws_400');
            }
            ...
            const reason = msg.reason || '未知';
            log('系统', `账号 ${  wrk.name  } 连接多次重试失败，尝试自动刷新 Code 后重启`, {
                accountId: String(accountId),
                accountName: wrk.name
            });
            ...
            addAccountLog('ws_reconnect_failed',
                `账号 ${  wrk.name  } 连接多次重试失败，尝试自动刷新 Code`,
                accountId, wrk.name, { reason });

            triggerCodeExpired(accountId, `ws_reconnect_failed:${reason}`);
