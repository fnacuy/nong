...
function createDataProvider(deps) {
    ...
    return {
        ...
        getDogInfo: (ref) => callWorkerApi(resolveAccountId(ref), 'getDogInfo'),
        setDogActive: (ref, dogId, active) => callWorkerApi(resolveAccountId(ref), 'setDogActive', dogId, active),

        // ========== Pet (宠物) ==========
        getPetStatus: (ref, friendGid) => callWorkerApi(resolveAccountId(ref), 'getPetStatus', friendGid || 0),
        getPetList: (ref) => callWorkerApi(resolveAccountId(ref), 'getPetList'),
        activateDog: (ref, dogTypeId) => callWorkerApi(resolveAccountId(ref), 'activateDog', dogTypeId),
        deployDog: (ref, dogTypeId) => callWorkerApi(resolveAccountId(ref), 'deployDog', dogTypeId),
        withdrawDog: (ref) => callWorkerApi(resolveAccountId(ref), 'withdrawDog'),
        getDogFoodList: (ref) => callWorkerApi(resolveAccountId(ref), 'getDogFoodList'),
        feedDog: (ref, itemId, count) => callWorkerApi(resolveAccountId(ref), 'feedDog', itemId, count),
        getGuardLogs: (ref, page, pageSize) => callWorkerApi(resolveAccountId(ref), 'getGuardLogs', page, pageSize),
        getGuardReward: (ref) => callWorkerApi(resolveAccountId(ref), 'getGuardReward'),
        claimGuardReward: (ref) => callWorkerApi(resolveAccountId(ref), 'claimGuardReward'),
        rawRpc: (ref, service, methodName, body) => callWorkerApi(resolveAccountId(ref), 'rawRpc', service, methodName, body),
        ...
        saveSettings: async (ref, settings) => {
            ...
            const patch = {
                ...
                immersiveFarming: s.immersiveFarming,
            ...
            return {
                ...
                immersiveFarming: store.getImmersiveFarming(id),
        ...
        broadcastPremiumStatus: (username) => {
            if (typeof broadcastPremiumStatus === 'function') {
                return broadcastPremiumStatus(username);
            }
            return 0;
        },
