...
async function submitRenew() {
  ...
  try {
    ...
    const res = await api.post('/api/user/renew', { cardCode: renewCardKey.value.trim() })
    if (res.data.ok) {
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div class="mt-1 border-t border-gray-100 pt-1 dark:border-gray-700">
          <button
            class="w-full flex items-center gap-2 px-4 py-2 text-sm transition-colors disabled:cursor-not-allowed hover:bg-gray-100/50 disabled:opacity-45 dark:hover:bg-gray-700/50"
