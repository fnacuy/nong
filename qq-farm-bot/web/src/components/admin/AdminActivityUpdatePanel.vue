...
<template>
  <section class="space-y-4">
    <div class="flex flex-col gap-3 border border-gray-200 rounded-lg bg-gray-50 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-700 dark:bg-gray-900/40">
      <div>
    ...
    <template v-if="report">
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
          ...
          <div class="mt-2 break-all text-sm text-gray-900 font-medium dark:text-white">
            在线 List + GetGroup
          </div>
        </div>
        <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
          ...
          <div class="mt-2 text-sm text-gray-900 dark:text-white">
            {{ formatTime(report.online?.scannedAt) }}
          </div>
          <div class="text-xs text-gray-500">
        ...
        <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
          ...
          <div class="mt-2 text-sm text-gray-900 font-medium dark:text-white">
            {{ report.online?.activities.length || 0 }} 个服务端节点
          </div>
          <div class="text-xs text-gray-500">
            仅统计 ActivityService 在线响应
          </div>
        </div>
      </div>

      <div class="border border-blue-200 rounded-lg bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
          ...
          <p class="mt-3 text-xs text-gray-400">
            共识别 {{ report.detectedActivityIds.length }} 个日期型活动 ID；候选项仍需协议样本确认。
          </p>
          <div v-if="report.analysis?.candidateGroups.length" class="mt-3 border-t border-gray-100 pt-3 space-y-2 dark:border-gray-700">
        ...
        <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
          ...
          <ul v-if="report.warnings.length" class="mt-3 text-sm text-amber-700 space-y-2 dark:text-amber-300">
          ...
          <div class="mt-3 text-xs text-gray-400">
            扫描时间：{{ formatTime(report.scannedAt) }}
          </div>
        </div>
      </div>
      ...
      <div class="border border-gray-200 rounded-lg p-4 dark:border-gray-700">
        ...
        <template v-else>
          <div class="grid mt-3 gap-3 sm:grid-cols-3">
            <div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-900/40">
              <div class="text-xs text-gray-500">
                服务端活动
              </div>
              <div class="mt-1 font-medium">
                {{ report.online.activities.length }} 个
              </div>
            </div>
            <div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-900/40">
              <div class="text-xs text-gray-500">
              ...
              <div class="mt-1 font-medium">
                {{ report.online.unknownActivityIds.length }} 个
              </div>
            </div>
            <div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-900/40">
              <div class="text-xs text-gray-500">
              ...
              <div class="mt-1 font-medium">
                {{ report.online.probes?.activityGroups || 0 }} 组 · {{ report.online.probes?.matched || 0 }} 节点
              </div>
            </div>
          </div>
          <div v-if="report.online.groups.length" class="mt-3 space-y-2">
            <div v-for="group in report.online.groups" :key="group.id" class="border border-gray-100 rounded-lg px-3 py-2 text-sm dark:border-gray-700">
              <div class="flex flex-wrap justify-between gap-2">
                <span class="text-gray-900 font-medium dark:text-white">{{ group.title || `活动 ${group.id}` }}</span>
      ...
      <div
        ...
        class="border border-amber-200 rounded-lg bg-amber-50 p-4 text-amber-900 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200"
      >
        ...
        <div class="mt-3 space-y-2">
          <div v-for="group in discoveredGroups" :key="group.id" class="rounded-lg bg-white/70 px-3 py-2 dark:bg-gray-900/40">
            ...
            <div class="mt-1 text-xs opacity-80">
              来源 List + GetGroup · {{ contentSummary(group) }}
            </div>
          </div>
        </div>
      </div>
