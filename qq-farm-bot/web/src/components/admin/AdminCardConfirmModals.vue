...
const props = defineProps<{
  ...
  batchUpdateCardsLoading: boolean
}>()
...
const showBatchUpdateCardsConfirm = defineModel<boolean>('showBatchUpdateCardsConfirm', { required: true })
const pendingBatchUpdate = defineModel<{ label: string, updates: Partial<Card> } | null>('pendingBatchUpdate', { required: true })
...
function closeBatchUpdateCardsConfirm() {
  if (!props.batchUpdateCardsLoading) {
...
<template>
  ...
  <ConfirmModal
    ...
    title="确认创建卡密"
    :message="`确定要创建${newCard.count > 1 ? `${newCard.count} 个` : ''}${newCard.type === 'quota' ? `额度卡密（+${newCard.quota || 0} 个账号额度）` : newCard.type === 'premium' ? `高级功能卡（${newCard.days === -1 ? '永久' : `${newCard.days || 0}${newCard.durationUnit === 'hour' ? '小时' : '天'}`}）` : `时间卡密（${newCard.days === -1 ? '永久' : `${newCard.days || 0}${newCard.durationUnit === 'hour' ? '小时' : '天'}`}，${newCard.accountLimit || 0} 个账号额度${newCard.claimable ? '，可免费领取' : '，不可免费领取'}）`}吗？${newCard.count > 1 ? '批量创建后会立即生成并导出本批卡密。' : '创建后卡密将立即可用。'}`"
    type="danger"
  ...
  <ConfirmModal
    :show="showBatchUpdateCardsConfirm"
    title="确认批量更新卡密"
    :message="`确定要${pendingBatchUpdate?.label || '更新'}当前选中的 ${selectedCardCount} 个卡密吗？`"
    type="danger"
    :loading="batchUpdateCardsLoading"
    confirm-text="确认执行"
    cancel-text="取消"
    @confirm="emit('batchUpdateCards')"
    @close="closeBatchUpdateCardsConfirm"
    @cancel="closeBatchUpdateCardsConfirm"
  />
</template>
