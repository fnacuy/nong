...
const props = defineProps<{
  ...
  showCodeCleanupModal: boolean
  codeCleanupLoading: boolean
  codeCleanupRunning: boolean
  codeCleanupConfig: {
    enabled: boolean
    retainDays: number
  ...
  <div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <BaseButton
          ...
          variant="secondary"
          size="sm"
          @click="emit('openCodeCleanup')"
        >
          <div class="i-carbon-reminder mr-2" />
          <span class="hidden sm:inline">清理失效Code</span>
          <span class="sm:hidden">失效Code</span>
        </BaseButton>
        <BaseButton
          variant="primary"
          size="sm"
    ...
    <Teleport to="body">
      ...
      <Transition name="confirm-modal">
        <div
          v-if="showCodeCleanupModal"
          class="confirm-modal-overlay"
          @click="!codeCleanupLoading && !codeCleanupRunning && emit('closeCodeCleanupModal')"
        >
          <div
            class="confirm-modal"
            data-tone="secondary"
          ...
          >
            <div class="confirm-modal-content">
              ...
              <p class="confirm-modal-message">
                定期清理两类账号：① 微信自动刷新 code 失效的账号（存在 wxid 且 code 为空、连续刷新失败超阈值或距最近成功刷新超时）；② 已停止运行且未绑定微信自动刷新的 QQ 端账号。删除不可恢复。
              </p>
            ...
            <div class="confirm-modal-actions">
              <BaseButton
                ...
                variant="secondary"
                :disabled="codeCleanupLoading || codeCleanupRunning"
                @click="emit('closeCodeCleanupModal')"
              >
                取消
              </BaseButton>
              <BaseButton
                class="confirm-modal-button"
                variant="danger"
                :loading="codeCleanupRunning"
                :disabled="codeCleanupLoading"
                @click="emit('runExpiredCodeCleanup')"
              >
                立即清理
              </BaseButton>
            </div>
            <div class="confirm-modal-actions">
              <BaseButton
                class="confirm-modal-button"
                variant="primary"
                :loading="codeCleanupLoading"
                :disabled="codeCleanupRunning"
                @click="emit('saveCodeCleanupConfig')"
              >
                保存配置
              </BaseButton>
            </div>
