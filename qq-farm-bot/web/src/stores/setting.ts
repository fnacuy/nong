...
export interface AutomationConfig {
  ...
  capital_mode?: boolean
  capital_mode_guard_seconds?: number
  capital_mode_dog_id?: number
...
export interface ImmersiveFarmingConfig {
  enabled?: boolean
  start?: string
  end?: string
}

export type PremiumQuietModeType = 'no_steal_no_help' | 'no_steal' | 'no_help'

export interface PremiumQuietModeSlot {
  id: string
  mode: PremiumQuietModeType
  start: string
  end: string
}

export interface PremiumQuietModeConfig {
  enabled?: boolean
...
export interface SettingsState {
  ...
  immersiveFarming: ImmersiveFarmingConfig
  premiumQuietMode: PremiumQuietModeConfig
...
export const useSettingStore = defineStore('setting', () => {
  const settings = ref<SettingsState>({
    ...
    immersiveFarming: { enabled: false, start: '00:00', end: '06:00' },
    premiumQuietMode: { enabled: false, slots: [] },
    automation: {},
  ...
  async function fetchSettings(accountId: string) {
    ...
    try {
      ...
      if (data && data.ok && data.data) {
        ...
        settings.value.immersiveFarming = d.immersiveFarming || { enabled: false, start: '00:00', end: '06:00' }
        settings.value.premiumQuietMode = d.premiumQuietMode || { enabled: false, slots: [] }
  ...
  async function saveSettings(accountId: string, newSettings: any) {
    ...
    try {
      const settingsPayload = {
        ...
        immersiveFarming: newSettings.immersiveFarming,
        premiumQuietMode: newSettings.premiumQuietMode,
  ...
  async function saveAutomation(accountId: string, patch: Record<string, unknown>) {
    if (!accountId)
      return { ok: false, error: '未选择账号' }
    try {
      const { data } = await api.post('/api/automation', patch, {
        headers: { 'x-account-id': accountId },
      })
      if (data && data.ok) {
        settings.value.automation = { ...settings.value.automation, ...patch }
        return { ok: true }
      }
