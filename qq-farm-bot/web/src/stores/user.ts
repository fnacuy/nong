...
export type PremiumStatus = TurboStatus
...
export function formatCardValue(card: Card) {
  ...
  if (card.type === 'turbo')
    return `${durationText}极速务农`
  if (card.type === 'premium')
    return `${durationText}高级功能`
...
export const useUserStore = defineStore('user', () => {
  ...
  const turbo = computed(() => userInfo.value?.turbo ?? null)

  // 极速务农权限是否有效（管理员/超级管理员永久有效）
  const hasTurboPermission = computed(() => {
    if (isAdmin.value)
      return true
    const t = turbo.value
    if (!t)
      return false
    if (t.isPermanent === true || t.days === -1 || t.durationValue === -1)
      return true
    return !!t.expiresAt && Date.now() < t.expiresAt
  })

  // 极速务农到期时间显示
  const turboExpireTimeText = computed(() => {
    if (isAdmin.value)
      return '永久有效'
    const t = turbo.value
    if (!t)
    ...
    if (!t.expiresAt)
      return '未激活'
    return new Date(t.expiresAt).toLocaleString('zh-CN')
  })

  const premium = computed(() => userInfo.value?.premium ?? null)

  // 高级功能权限是否有效（管理员/超级管理员永久有效）
  const hasPremiumPermission = computed(() => {
    if (isAdmin.value)
      return true
    const p = premium.value
    if (!p)
      return false
    if (p.isPermanent === true || p.days === -1 || p.durationValue === -1)
      return true
    return !!p.expiresAt && Date.now() < p.expiresAt
  })

  // 高级功能到期时间显示
  const premiumExpireTimeText = computed(() => {
    if (isAdmin.value)
      return '永久有效'
    const p = premium.value
    if (!p)
    ...
    if (!p.expiresAt)
      return '未激活'
    return new Date(p.expiresAt).toLocaleString('zh-CN')
  })
    ...
    try {
      ...
      if (res.data.ok) {
        ...
        userInfo.value = {
          ...
          turbo: res.data.data.turbo ?? null,
          premium: res.data.data.premium ?? null,
        }
      }
      return res.data
    }
  ...
  async function renew(cardCode: string) {
    ...
    if (res.data.ok) {
      ...
      if (userInfo.value) {
        ...
        userInfo.value.turbo = res.data.data.turbo ?? null
        userInfo.value.premium = res.data.data.premium ?? null
      }
    }
    return res.data
  }
  ...
  async function updateCardsBatch(codes: string[], updates: Partial<Card>, payload?: { confirmed?: boolean, confirmText?: string }) {
    const res = await api.post('/api/admin/cards/batch-update', { codes, ...updates, ...payload })
    return res.data
  }
