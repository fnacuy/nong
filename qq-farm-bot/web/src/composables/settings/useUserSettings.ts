...
export function useUserSettings(showAlert: (message: string, type?: AlertType) => void) {
  ...
  const showActivateCardModal = ref(false)
  const showActivateCardConfirm = ref(false)
  const activateCardLoading = ref(false)
  const activateUsername = ref('')
  const activateCardCode = ref('')
  ...
  function openActivateCardModal() {
    ...
    activateCardCode.value = ''
    showActivateCardModal.value = true
  }

  function requestActivateCard() {
    if (!activateUsername.value.trim()) {
      showAlert('请输入要激活卡密的用户名', 'danger')
      return
    }
    if (!activateCardCode.value.trim()) {
      showAlert('请输入卡密', 'danger')
      return
    }
    showActivateCardConfirm.value = true
  }

  async function confirmActivateCard() {
    const username = activateUsername.value.trim()
    const cardCode = activateCardCode.value.trim()
    if (!username || !cardCode) {
      showAlert('请输入完整的用户名和卡密', 'danger')
      return
    }
    activateCardLoading.value = true
    try {
      const res = await userStore.renewUser(username, cardCode, { confirmed: true })
      if (res.ok) {
        showAlert(`已为用户 ${username} 激活卡密`, 'primary')
        showActivateCardConfirm.value = false
        showActivateCardModal.value = false
        activateUsername.value = ''
        activateCardCode.value = ''
      }
      else {
        showAlert(res.error || '激活卡密失败', 'danger')
      }
    }
    catch (e: any) {
      showAlert(e?.response?.data?.error || e?.message || '激活卡密失败', 'danger')
    }
    finally {
      activateCardLoading.value = false
    }
  }
