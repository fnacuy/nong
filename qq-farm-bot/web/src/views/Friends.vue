...
async function handleDeleteBlacklistFriend(item: any) {
  if (!currentAccountId.value)
    return
  ...
  confirmAction(
    `确定要删除好友「${name}」吗？此操作不可恢复，将直接从好友列表中移除该好友，并从黑名单中移出。`,
    async () => {
      const result = await friendStore.deleteFriend(currentAccountId.value!, gid)
      if (result.ok) {
        friends.value = friends.value.filter(f => Number(f.gid) !== gid)
        blacklist.value = blacklist.value.filter(b => Number(b.gid) !== gid)
        toast.success(result.message || `已删除好友: ${name}`)
      }
      else {
        toast.error(result.error || '删除好友失败')
...
async function handleUpdateBlacklistItem(gid: number, opts: { skipSteal?: boolean, skipHelp?: boolean }) {
  if (!currentAccountId.value)
    return
  await friendStore.updateBlacklistItem(currentAccountId.value, gid, opts)
}
      ...
      <div v-else-if="activeTab === 'blacklist'" class="space-y-4">
        ...
        <div v-else class="space-y-2">
          ...
          >
            <div class="min-w-0 flex items-center gap-3">
              ...
              <div class="min-w-0">
                ...
                <div class="mt-1.5 flex flex-wrap items-center gap-3">
                  ...
                  <span
                    v-if="!item.skipSteal && !item.skipHelp"
                    class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500 dark:bg-gray-700 dark:text-gray-300"
                  >
                    当前不跳过任何操作
                  </span>
                </div>
            ...
            <div class="w-full flex shrink-0 gap-2 sm:w-auto">
              <button
                class="flex-1 rounded bg-red-100 px-3 py-1.5 text-sm text-red-600 sm:flex-none dark:bg-red-900/30 hover:bg-red-200 dark:text-red-400 dark:hover:bg-red-900/50"
                @click="handleRemoveFromBlacklist(item.gid)"
              >
                移出黑名单
              </button>
              <button
                class="flex-1 rounded bg-gray-100 px-3 py-1.5 text-sm text-gray-600 sm:flex-none dark:bg-gray-700 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-600"
                @click="handleDeleteBlacklistFriend(item)"
              >
                删除好友
              </button>
            </div>
