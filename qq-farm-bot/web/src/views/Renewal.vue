...
const cardUsageSummary = computed(() => {
  ...
  if (cardInfo.value?.type === 'turbo')
    return `已识别为极速务农卡，预计激活 ${formatTimeDuration(cardInfo.value)} 极速务农（只帮护主犬）权限`
...
const pageSummary = computed(() => {
  ...
  if (cardInfo.value) {
    ...
    if (cardInfo.value.type === 'turbo')
      return '当前卡密可激活极速务农（只帮护主犬）权限，提交后自动控制中的对应开关即可使用。'
    const limit = Number(cardInfo.value.accountLimit)
    if (Number.isFinite(limit) && limit > 0)
      return '当前卡密可延长账号时长，并将账号额度设置为卡密指定数量。'
    return '当前卡密可用于延长账号时长，提交后会立即更新有效期。'
  }
  return '时间卡会延长有效期，额度卡会增加可添加账号数量，极速务农卡会激活极速务农权限。'
})
const cardTypeLabel = computed(() => {
  ...
  if (cardInfo.value.type === 'quota')
    return '额度卡'
  return cardInfo.value.type === 'turbo' ? '极速务农卡' : '时间卡'
})
    ...
    <div class="glass-card rounded-3xl p-8">
      ...
      <div class="grid mb-6 gap-3 sm:grid-cols-2">
        <div class="rounded-2xl bg-gray-50 px-4 py-3 text-sm text-gray-600 dark:bg-gray-900/40 dark:text-gray-300">
          {{ isLoggedIn ? '当前已登录，将直接为当前账号续费并同步刷新本地账号信息。' : '时间卡会延长账号有效期；额度卡会增加可添加账号数量；极速务农卡会激活极速务农权限。若返回失败，优先核对用户名是否与当前登录名一致。' }}
        </div>
        <div
          ...
          <div v-if="cardInfo" class="border border-blue-200 rounded-2xl bg-blue-50 p-4 text-sm dark:border-blue-800 dark:bg-blue-900/20">
            <div class="mb-3 flex items-center justify-between gap-3">
              ...
              <span
                ...
                :class="cardInfo.type === 'quota' ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/60 dark:text-orange-200' : (cardInfo.type === 'turbo' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200' : 'bg-blue-100 text-blue-800 dark:bg-blue-800/60 dark:text-blue-100')"
              >
                {{ cardTypeLabel }}
              </span>
            </div>
            ...
            <div class="grid gap-3 sm:grid-cols-2">
              ...
              <div class="rounded-xl bg-white/70 px-3 py-2 text-gray-700 dark:bg-gray-950/20 dark:text-gray-200">
                ...
                <div class="mt-1 font-semibold">
                  {{ cardInfo.type === 'quota' ? '适合补账号额度' : (cardInfo.type === 'turbo' ? '适合激活极速务农' : '适合补账号时长') }}
                </div>
              </div>
            </div>
          </div>
